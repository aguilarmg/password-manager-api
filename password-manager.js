...
var keychainClass = function() {
  ...
  keychain.init = function(password) {
    ...
    let kdf_salt = randomBitarray(128);
    console.log("KDF salt: ", kdf_salt);

    //let master_key = KDF(password, kdf_salt);
    priv.data.master_key = KDF(password, kdf_salt);
    console.log("Master Key generated by PBKDF2: ", priv.secrets.master_key);

    //let hmac_key = HMAC(master_key, randomBitarray(128));
    priv.secrets.hmac_key = HMAC(priv.data.master_key, randomBitarray(128));
    console.log("HMAC Key generated by HMAC: ", priv.secrets.hmac_key);

    //let aes_key = HMAC(master_key, randomBitarray(128));
    priv.secrets.aes_key = HMAC(priv.data.master_key, randomBitarray(128));
    console.log("AES-GCM Key generated by HMAC: ", priv.secrets.aes_key);
  ...
  keychain.dump = function() {
    ...
    return [ json_keychain, priv.secrets.sha256_checksum ];
  };
